...
//BT_S : [CONBT-3590][CASE#1045060] add recovery patch for hcimodule start fail
#define RECOVERY_BLOCK_READ TRUE
#if defined(RECOVERY_BLOCK_READ) && (RECOVERY_BLOCK_READ == TRUE)
#include <sys/epoll.h>
#define MAX_EVENTS  64
#define INVALID_FD (-1)
#define MAX_WAIT_TIME 3000
#endif
//BT_E : [CONBT-3590][CASE#1045060] add recovery patch for hcimodule start fail

//BT_S : [ZTWOGFOURM-1302] workaround for sepolicy issue, [START]
#include <selinux/selinux.h>
#include <selinux/label.h>
#include <selinux/android.h>
//BT_E : [ZTWOGFOURM-1302] workaround for sepolicy issue, [END]


#define UIM_DEBUG 1

#ifdef UIM_DEBUG          /* limited debug messages */
...
//BT_S : [CONBT-2564][CSP#962718] improve a2dp chopping using vendor command
#define BRCM_LGE_VSC_CHIP_CHECK_PROP    "service.brcm.bt.chip_check"
//BT_E : [CONBT-2564][CSP#962718] improve a2dp chopping using vendor command
...
//BT_S : [CONBT-4302][CSP#1115632] Static Analysis Result
    strncat(hw_cfg_string, p_conf_value, sizeof(hw_cfg_string)-strlen(hw_cfg_string)-1);
//BT_E : [CONBT-4302][CSP#1115632] Static Analysis Result
...
//BT_S : [CONBT-4318][CSP#1117714] Static Analysis Result
    strncpy(hci_snoop_path, p_conf_value, sizeof(hci_snoop_path) - 1);
    hci_snoop_path[sizeof(hci_snoop_path) - 1] = '\0';
//BT_E : [CONBT-4318][CSP#1117714] Static Analysis Result
    UIM_DBG("%s = %s", p_conf_name, p_conf_value);
    return 0;
}
#endif

//BT_S : [CONBT-2025] HCI_BTSNOOP_LOG (V4L2)
char hci_logfile[256];
static int is_btsnoop_enabled() {
    char str[256] = {0, };
    int len = property_get("product.lge.bt.snoop", str, "0");
    if (len > 1) {
        memset(hci_logfile, 0, sizeof(hci_logfile));
        strcpy(hci_logfile, str);
        return 1;
    }
    return 0;
}
//BT_E : [CONBT-2025] HCI_BTSNOOP_LOG (V4L2)
...
#if 0//def ANDROID   /* library for android to do insmod/rmmod  */
/*****************************************************************************/
...
static int insmod(const char *filename, const char *args)
{
    ...
    UIM_DBG("module %x %s", module,filename);
    if (!module)
    {
...
//BT_S : [CONBT-4421] ENG_MODE_SELECT_PATCHRAM (initial BT on after reboot)
#if 0 //G6_POS_BRINGUP
    char str[256] = {0, };
#endif
//BT_E : [CONBT-4421] ENG_MODE_SELECT_PATCHRAM (initial BT on after reboot)
...
//BT_S : [CONBT-4421] ENG_MODE_SELECT_PATCHRAM (initial BT on after reboot)
#if 0 //G6_POS_BRINGUP
                    property_get("persist.service.bdroid.patchram", str, "");
                    if (strlen(str) > 0) {
                        UIM_DBG("[BTUI] wanted: %s", str);
                        if (strcmp(str, dp->d_name) == 0) {
                            UIM_ERR("[BTUI] selected: %s", dp->d_name);
                            strcpy(p_chip_id_str, dp->d_name);
                            retval = TRUE;
                            break;
                        } else {
                            UIM_DBG("[BTUI] not wanted: %s", dp->d_name);
                            continue;
                        }
                    }
#endif
...
//BT_S : [CONBT-2564][CSP#962718] improve a2dp chopping using vendor command
    // latest BT controllers can't use opcode '0xFCDA' for LGE VSC.
    // so, need to use opcode '0xFD9D' instead of '0xFCDA' when using them.
    if((strcmp(p_chip_id_str, "BCM4358A3") == 0) || (strcmp(p_chip_id_str, "BCM4359C0") == 0))
    {
        UIM_DBG("proc_read_local_name, %s : true", BRCM_LGE_VSC_CHIP_CHECK_PROP);
        if(property_set(BRCM_LGE_VSC_CHIP_CHECK_PROP, "true") < 0)
            UIM_DBG("Failed to set %s",BRCM_LGE_VSC_CHIP_CHECK_PROP);
    }
    else
    {
        UIM_DBG("proc_read_local_name, %s : false",BRCM_LGE_VSC_CHIP_CHECK_PROP);
        if(property_set(BRCM_LGE_VSC_CHIP_CHECK_PROP, "false") < 0)
            UIM_DBG("Failed to set: %s", BRCM_LGE_VSC_CHIP_CHECK_PROP);
    }
//BT_E : [CONBT-2564][CSP#962718] improve a2dp chopping using vendor command

//BT_S : [CONBT-1288] Undef CHANGE_DUT_PATCHRAM
//#if (defined(CHANGE_CHIPSET_NAME) && CHANGE_CHIPSET_NAME == TRUE)
        if(strcmp(p_chip_id_str, "BCM4335C0") == 0){
            strcpy(p_chip_id_str, "BCM4339");
            UIM_DBG("Change chipset Name: %s", p_name);
        }
//#endif
//BT_E : [CONBT-1288] Undef CHANGE_DUT_PATCHRAM
...
//BT_S : [CONBT-4421] ENG_MODE_SELECT_PATCHRAM (BT off and on)
#if 0 //G6_POS_BRINGUP
            char str[256] = {0, };
            property_get("persist.service.bdroid.patchram", str, "");
            if (strlen(str) > 0) {
                if (proc_read_local_name(local_chip_name) == TRUE) { //to avoid BT on fail
                    strcpy(fw_patchfile_name, local_chip_name);
                }
            }
#endif
//BT_E : [CONBT-4421] ENG_MODE_SELECT_PATCHRAM (BT off and on)
            UIM_DBG("complete fw file name = %s", fw_patchfile_name);
        }
        else if(proc_read_local_name(local_chip_name) == TRUE) {
            UIM_DBG("complete fw file name (read_local_name) = %s", local_chip_name);
            strcpy(fw_patchfile_name, local_chip_name);
        }
        else {
...
//BT_S : [ZTWOGFOURM-1302] workaround for sepolicy issue, [START]
    if(selinux_android_restorecon(BDADDR_SYSFS_ENTRY, SELINUX_ANDROID_RESTORECON_RECURSE)){
        UIM_ERR("%s, selinux_android_restorecon=%s", __func__, bdstr);
    }

    if(selinux_android_restorecon(FW_PATCHFILE_SYSFS_ENTRY, SELINUX_ANDROID_RESTORECON_RECURSE)){
        UIM_ERR("%s, selinux_android_restorecon bus =%s", __func__, bdstr);
    }

    if(selinux_android_restorecon(LDISC_VENDOR_PARAMS, SELINUX_ANDROID_RESTORECON_RECURSE)){
        UIM_ERR("%s, selinux_android_restorecon bus =%s", __func__, bdstr);
    }
//BT_E : [ZTWOGFOURM-1302] workaround for sepolicy issue, [END]
...
//BT_S : Merge for FM selinux issue
    if (property_set("bluetooth.fm.restorecon", "1") < 0) {
        UIM_ERR("un-available set fm restorecon property");
    }
//BT_E : Merge for FM selinux issue
...
//BT_S : [CONBT-3304] Delete unnecessary code
/*
...
*/
//BT_E : [CONBT-3304] Delete unnecessary code
...
//BT_S : [CONBT-2025] HCI_BTSNOOP_LOG (V4L2)
            if (is_btsnoop_enabled()) {
                ...
                memset(hci_snoop_path, 0, sizeof(hci_snoop_path));
//BT_S : [CONBT-4318][CSP#1117714] Static Analysis Result
                strncpy(hci_snoop_path, hci_logfile, sizeof(hci_snoop_path) - 1);
                hci_snoop_path[sizeof(hci_snoop_path) - 1] = '\0';
//BT_E : [CONBT-4318][CSP#1117714] Static Analysis Result
            }
//BT_E : [CONBT-2025] HCI_BTSNOOP_LOG (V4L2)
