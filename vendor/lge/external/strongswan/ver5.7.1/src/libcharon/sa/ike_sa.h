...
struct ike_sa_t {
	...
	/**
	 * Sends a DPD request now to the peer.
	 *
	 * To check if a peer is still alive, periodic
	 * empty INFORMATIONAL messages are sent if no
	 * other traffic was received.
	 ...
	 *						- DESTROY_ME, if peer did not respond
	 */
	status_t (*send_dpd_now) (ike_sa_t *this);

	/**
	 * Sends a DPD request now per connection to the peer.
	 *
	 * To check if a peer is still alive, periodic
	 * empty INFORMATIONAL messages are sent if no
	 * other traffic was received.
	 ...
	 *						- DESTROY_ME, if peer did not respond
	 */
	status_t (*send_dpd_now_per_conn) (ike_sa_t *this, int retransmit_tries, int retransmit_timeout);
	...
	void (*set_imei) (ike_sa_t *this, const char *imei);
