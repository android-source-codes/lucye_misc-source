	...
	private_traffic_selector_t *this, host_t *host)
{
	if (is_host(this, NULL))
	{
		...
		else
		{
			...
			{
				this->netbits = max(this->netbits, host->get_netbits(host));
			}
			memcpy(this->from, from.ptr, from.len);
			memcpy(this->to,   from.ptr, from.len);
			calc_range(this, this->netbits);
		}
		this->dynamic = FALSE;
	}
}

METHOD(traffic_selector_t, has_full_range, bool,
		private_traffic_selector_t *this, ts_type_t cmp_type)
{
	size_t len = 0;
	u_int32_t from4[1];
	u_int32_t from6[4];
	...
	if (this != NULL && this->type == TS_IPV4_ADDR_RANGE && this->type == cmp_type) {
		len = 4;
		memset(from4, 0x00, sizeof(from4));
		memset(to4, 0xFF, sizeof(to4));
		if (memeq(this->from, from4, len) && memeq(this->to, to4, len)) {
			return TRUE;
		}
	} else if (this != NULL && this->type == TS_IPV6_ADDR_RANGE && this->type == cmp_type) {
		len = 16;
		memset(from6, 0x00, sizeof(from6));
		memset(to6, 0xFF, sizeof(to6));
		if (memeq(this->from, from6, len) && memeq(this->to, to6, len)) {
			return TRUE;
		}
	}
	return FALSE;
		...
		private_traffic_selector_t *this)
{
	...
	if (this != NULL && this->type == TS_IPV4_ADDR_RANGE) {
		...
		this->netbits = 0;
	} else if (this != NULL && this->type == TS_IPV6_ADDR_RANGE) {
		memset(this->from6, 0x00, sizeof(this->from6));
		memset(this->to6, 0xFF, sizeof(this->to6));
		this->netbits = 0;
	}
...
METHOD(traffic_selector_t, set_ts_single_addr, void,
	private_traffic_selector_t *this, host_t *host)
{
	this->type = host->get_family(host) == AF_INET ?
			TS_IPV4_ADDR_RANGE : TS_IPV6_ADDR_RANGE;
	...
	else
	{
		...
		if ((host->get_family(host) == AF_INET6))
		{
			this->netbits = max(128, host->get_netbits(host));
		}
		else {
			this->netbits = max(32, host->get_netbits(host));
		}
		memcpy(this->from, from.ptr, from.len);
		memcpy(this->to,   from.ptr, from.len);
		calc_range(this, this->netbits);
...
METHOD(traffic_selector_t, set_ts_ipv6_range_64, void,
	private_traffic_selector_t *this)
{
	if (this->type == TS_IPV6_ADDR_RANGE) {
		calc_range(this, 64);
	}
}
						...
						ts_type_t type, u_int16_t from_port, u_int16_t to_port)
{
	...
	INIT(this,
		.public = {
			...
			.has_full_range = _has_full_range,
			/* 2017-01-02 sy.yun@lge.com LGP_DATA_IWLAN_FORCE_ANYADDR_TSR_WHILE_REKEY [START] */
			.set_ts_any_addr = _set_ts_any_addr,
			/* 2017-01-02 sy.yun@lge.com LGP_DATA_IWLAN_FORCE_ANYADDR_TSR_WHILE_REKEY [END] */
			.set_ts_single_addr = _set_ts_single_addr,
			.set_ts_ipv6_range_64 = _set_ts_ipv6_range_64,
