...
static int trigger_dpd_now()
{
	stroke_msg_t msg;

	msg.type = STR_DPD_NOW;
	msg.length = offsetof(stroke_msg_t, buffer);
	return send_stroke_msg(&msg);
}

static int trigger_dpd_now_per_conn(char *conn_name, int retransmit_tries, int retransmit_timeout)
{
	stroke_msg_t msg;

	msg.type = STR_DPD_NOW_PER_CONN;
	msg.length = offsetof(stroke_msg_t, buffer);
	msg.dpd_conn.name = push_string(&msg, conn_name);
	msg.dpd_conn.retransmit_tries = retransmit_tries;
	msg.dpd_conn.retransmit_timeout = retransmit_timeout;
	...
	switch (token->kw)
	{
		...
		case STROKE_DPD:
			...
			if (argc == 3)
				...
				res = trigger_dpd_now_per_conn(argv[3], 0, 0);
			else if (argc == 6)
				res = trigger_dpd_now_per_conn(argv[3], atoi(argv[4]), atoi(argv[5]));
			else
			/* 2016-07-02 protocol-iwlan@lge.com LGP_DATA_IWLAN_DPD_NOW [END] */
			res = trigger_dpd();
			break;
