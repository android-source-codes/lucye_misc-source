...
static char* get_string(private_message_t *this, char *buf, int len)
{
	...
	while (enumerator->enumerate(enumerator, &payload))
	{
		...
		if (payload->get_type(payload) == FRAGMENT_V1)
		{
...
static encrypted_payload_t* wrap_payloads(private_message_t *this)
{
	...
	{
		encrypted = encrypted_payload_create(ENCRYPTED_V1);
	}
	else
	{
		encrypted = encrypted_payload_create(ENCRYPTED);
	}
	...
	enumerator_t **fragments)
{
	...
	message_t *fragment;
	packet_t *packet;
	payload_type_t next = NO_PAYLOAD;
	u_int16_t num, count;
	host_t *src, *dst;
	chunk_t data;
	status_t status;
	...
	if (is_encoded(this))
	{
		if (this->major_version == IKEV2_MAJOR_VERSION)
		{
			encrypted = (encrypted_payload_t*)get_payload(this, ENCRYPTED);
		}
...
static status_t parse_payloads(private_message_t *this)
{
	...
	while (type != NO_PAYLOAD)
	{
		...
		if (type == ENCRYPTED || type == PLV2_FRAGMENT)
		{
	...
	private_message_t *this, message_t *message)
{
	...
	payload = (fragment_payload_t*)message->get_payload(message, FRAGMENT_V1);
	if (!payload)
	{
		return INVALID_ARG;
	}
...
message_t *message_create_defrag(message_t *fragment)
{
	...
	if (!fragment->get_payload(fragment, FRAGMENT_V1) &&
