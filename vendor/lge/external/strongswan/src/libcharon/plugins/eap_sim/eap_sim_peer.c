								  ...
								  simaka_message_t *in, eap_payload_t **out, char *name)
{
	...
	DBG1(DBG_LIB, "rands.len: %d", rands.len);
	...
	while (rands.len >= SIM_RAND_LEN)
	{
		if (!this->mgr->card_get_triplet(this->mgr, this->permanent,
										 rands.ptr, sres.ptr, kc.ptr, name))
		{
	...
	DBG1(DBG_LIB, "ID: %Y", id);
	DBG1(DBG_LIB, "kcs: %B", &kcs);
	DBG1(DBG_LIB, "this->nonce: %B", &(this->nonce));
	DBG1(DBG_LIB, "this->version_list: %B", &(this->version_list));
	DBG1(DBG_LIB, "version: %B", &version);
#endif
	data = chunk_cata("cccc", kcs, this->nonce, this->version_list, version);

#ifndef DEBUG_LOG_DISABLE
	DBG1(DBG_LIB, "data: %B", &data);
#endif
