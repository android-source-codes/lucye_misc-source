...
METHOD(stroke_list_t, trigger_dpd, void,
	private_stroke_list_t *this, stroke_msg_t *msg, FILE *out)
{
	enumerator_t *enumerator;
	ike_sa_t *ike_sa;

	enumerator = charon->controller->create_ike_sa_enumerator(charon->controller, TRUE);
	while (enumerator->enumerate(enumerator, &ike_sa))
	{
		bool ike_printed = FALSE;
		enumerator_t *children = ike_sa->create_child_sa_enumerator(ike_sa);
		ike_sa->send_dpd(ike_sa);
	}
	enumerator->destroy(enumerator);
}

/* 2016-07-02 protocol-iwlan@lge.com LGP_DATA_IWLAN_DPD_NOW [START] */
METHOD(stroke_list_t, trigger_dpd_now, void,
	private_stroke_list_t *this, stroke_msg_t *msg, FILE *out)
{
	enumerator_t *enumerator;
	ike_sa_t *ike_sa;
	DBG1(DBG_CFG, "trigger_dpd_now");
	enumerator = charon->controller->create_ike_sa_enumerator(charon->controller, TRUE);
	while (enumerator->enumerate(enumerator, &ike_sa))
	{
		bool ike_printed = FALSE;
		...
		{
			if (strcmp(ike_sa->get_name(ike_sa), msg->dpd_conn.name) == 0)
			{
				DBG1(DBG_CFG, "trigger_dpd_now name for ike_sa: %s ", ike_sa->get_name(ike_sa));
				ike_sa->send_dpd_now_per_conn(ike_sa, msg->dpd_conn.retransmit_tries, msg->dpd_conn.retransmit_timeout);
			}
		}
		else
		{
			DBG1(DBG_CFG, "trigger_dpd_now name for ike_sa: %s ", ike_sa->get_name(ike_sa));
			ike_sa->send_dpd_now(ike_sa);
		}
	}
	enumerator->destroy(enumerator);
}
...
stroke_list_t *stroke_list_create(stroke_attribute_t *attribute)
{
	...
	INIT(this,
		.public = {
			...
			.leases = _leases,
			.destroy = _destroy,
			.trigger_dpd = _trigger_dpd,
			/* 2016-07-02 protocol-iwlan@lge.com LGP_DATA_IWLAN_DPD_NOW [START] */
			.trigger_dpd_now = _trigger_dpd_now,
			/* 2016-07-02 protocol-iwlan@lge.com LGP_DATA_IWLAN_DPD_NOW [END] */
		},
