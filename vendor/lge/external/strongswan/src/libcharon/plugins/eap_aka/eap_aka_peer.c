...
static status_t process_challenge(private_eap_aka_peer_t *this,
								  simaka_message_t *in, eap_payload_t **out, char *name)
{
	simaka_message_t *message;
	enumerator_t *enumerator;
	simaka_attribute_t type;
	...
	status = this->mgr->card_get_quintuplet(this->mgr, this->permanent,
									rand.ptr, autn.ptr, ck, ik, res, &res_len, name);
	...
	private_eap_aka_peer_t *this, eap_payload_t *in, eap_payload_t **out)
{
	...
	switch (message->get_subtype(message))
	{
		case AKA_IDENTITY:
			...
			break;
		case AKA_CHALLENGE:
			status = process_challenge(this, message, out, in->get_name(in));
			break;
									...
									identification_t *peer)
{
	...
	/*
	 * Remove MAC addr from IDi to build EAP ID if IDi type = ID_RFC822_ADDR_MAC_EAP_NO_MAC.
	 */
	chunk_t id_i = peer->get_encoding(peer);
	char *id_nomac;
	int i, len = 0;
	bool skip = false;
	...
	if (strchr(id_i.ptr, ':'))
	{
		id_nomac = (char *)malloc(id_i.len + 1);

		/* ID_RFC822_ADDR_MAC format: <IMSI>@xx-xx-xx-xx-xx-xx:<EPDG FQDN> */
		DBG1(DBG_KEY, "ID: %B", &id_i);
		for (i = 0; i < id_i.len; i++)
		{
			if (!skip)
			{
				id_nomac[len++] = id_i.ptr[i];
			}
			if (id_i.ptr[i] == '@')
			{
				skip = true;
		...
		DBG1(DBG_KEY, "new EAP Auth ID (no MAC): %s, len: %d, strlen: %d", id_nomac, len, strlen(id_nomac));
		this->permanent = identification_create_from_string(id_nomac);
		free(id_nomac);
	}
	else
	{
