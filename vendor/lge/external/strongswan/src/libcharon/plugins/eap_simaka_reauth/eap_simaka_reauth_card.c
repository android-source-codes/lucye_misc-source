...
METHOD(simaka_card_t, get_quintuplet, status_t,
	...
	char ik[AKA_IK_LEN], char res[AKA_RES_MAX], int *res_len, char* name)
{
	char txbuf[MAX_BUF_LEN];
	char rxbuf[MAX_BUF_LEN];
	...
	sprintf(txbuf, "AT+EAUTH=\"%s\",\"%s\",\"%s\"\r", name, rand_hexstr, autn_hexstr);
...
#ifndef DEBUG_LOG_DISABLE
	DBG2(DBG_IKE, "Got response: %s", rxbuf);
#endif
