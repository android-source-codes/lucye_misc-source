...
static configuration_attribute_t *build_pcscf(host_t *pcscf, int slotid)
{
	...
	{
		type = get_cust_setting_int_by_slotid(slotid, CUST_PCSCF_IP4_VALUE);
	...
	{
		type = get_cust_setting_int_by_slotid(slotid, CUST_PCSCF_IP6_VALUE);
...
{
	configuration_attribute_type_t type = get_cust_setting_int_by_slotid(slotid, CUST_IMEI_CP);
	chunk_t chunk;
	size_t i;
	u_char *bin = malloc(9);
	...
	for (i = 0; i < strlen(imei); i += 2) {
		if (i == 14 && strlen(imei) == 15) {
	...
	switch (ca->get_type(ca))
	{
		...
		case INTERNAL_IP6_ADDRESS:
		{
			...
			else
			{
				...
				if (get_cust_setting_bool_by_slotid(slotid, USE_CFG_VIP) && ip)
				{
		...
		default:
		{
			if (ca->get_type(ca) == get_cust_setting_int(CUST_PCSCF_IP4_VALUE) || ca->get_type(ca) == get_cust_setting_int_by_slotid(1, CUST_PCSCF_IP4_VALUE)) {
				family = AF_INET;
				addr = ca->get_chunk(ca);
				if (addr.len == 0)
				{
			...
			else if (ca->get_type(ca) == get_cust_setting_int(CUST_PCSCF_IP6_VALUE) || ca->get_type(ca) == get_cust_setting_int_by_slotid(1, CUST_PCSCF_IP6_VALUE)) {
				addr = ca->get_chunk(ca);
				if (addr.len == 0)
				{
					ip = host_create_any(family);
			...
			else if (ca->get_type(ca) == get_cust_setting_int(CUST_IMEI_CP) || ca->get_type(ca) == get_cust_setting_int_by_slotid(1, CUST_IMEI_CP)) {
	...
	private_ike_config_t *this, message_t *message)
{
	...
	{	/* in first IKE_AUTH only */
		...
		if (pcscfs->get_count(pcscfs))
		{
			...
			{
				int slotid = get_slotid(this->ike_sa->get_name(this->ike_sa));
				cp->add_attribute(cp, build_pcscf(host, slotid));
			}
			enumerator->destroy(enumerator);
		}
