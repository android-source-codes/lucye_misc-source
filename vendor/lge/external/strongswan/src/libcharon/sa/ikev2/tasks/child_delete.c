	...
	while (payloads->enumerate(payloads, &payload))
	{
		if (payload->get_type(payload) == DELETE)
		{
			...
			while (spis->enumerate(spis, &spi))
			{
				...
				if (slotid != 0) {
					sprintf(prop_name, "net.handover.flag.%d", slotid);
				} else {
					sprintf(prop_name, "net.handover.flag");
				}
				if (property_get_bool(prop_name, 0)) {
					DBG1(DBG_IKE, "*************delay 1 seconds for handover");
					sleep(1);
				}
#endif
