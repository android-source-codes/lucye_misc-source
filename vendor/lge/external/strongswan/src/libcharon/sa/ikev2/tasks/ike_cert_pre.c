						 ...
						 cert_encoding_t encoding, bool *first)
{
	...
	else if (encoding == ENC_X509_HASH_AND_URL)
	{
	...
	} else if (encoding == ENC_X509_HASH_AND_URL_BUNDLE) {
		/* we fetch the certificate not yet, but only if
		 * it is really needed during authentication */
		url = payload->get_url(payload);
		if (!url)
		...
		if (first)
		{	/* first URL is for an end entity certificate */
			DBG1(DBG_IKE, "received hash-and-url for end entity cert \"%s\"", url);
			auth->add(auth, AUTH_HELPER_BUNDLE_HASH_URL, url);
			first = FALSE;
		}
		else
		{
		...
		DBG1(DBG_IKE, "Done of URL fetch: \"%s\"", url);
	}
}
...
static void build_certreqs(private_ike_cert_pre_t *this, message_t *message)
{
	...
	if (!get_cust_setting_bool_by_slotid(slotid, IS_CERTIFICATE_USED))
	{
	...
	else if (get_cust_setting_bool_by_slotid(slotid ,IKE_HASHANDURL))
	{
