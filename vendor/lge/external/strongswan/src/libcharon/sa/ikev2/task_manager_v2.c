	...
	private_task_manager_t *this, u_int32_t message_id)
{
		...
		array_count(this->initiating.packets))
	{
		...
		bool is_dpd = FALSE;
		bool aqure_wakelock = FALSE;

		/* check if we are retransmitting a DPD routability check */
		enumerator = array_create_enumerator(this->active_tasks);
		while (enumerator->enumerate(enumerator, (void*)&task))
		{
			if (task->get_type(task) == TASK_IKE_DPD)
			{
				/* 2016-07-02 protocol-iwlan@lge.com LGP_DATA_IWLAN_DPD_NOW [START] */
				t_dpd = (ike_dpd_t*)task;
				/* 2016-07-02 protocol-iwlan@lge.com LGP_DATA_IWLAN_DPD_NOW [END] */
				is_dpd = TRUE;
				break;
			}
		...
		if (mobike == NULL)
		{
			...
			/* 2016-07-02 protocol-iwlan@lge.com LGP_DATA_IWLAN_DPD_NOW [START] */
			if (t_dpd != NULL && is_dpd && t_dpd->get_retrans_tries(t_dpd) > 0 && t_dpd->get_retrans_timeout(t_dpd) > 0)
			{
				if (this->initiating.retransmitted <= t_dpd->get_retrans_tries(t_dpd))
				{
			...
			{
				/* 2016-03-02 protocol-iwlan@lge.com LGP_DATA_IWLAN [START] */
				// Kernel don't sleep if there is alarm expire in 2 seconds.
				...
				if ((this->initiating.retransmitted == 0) &&
						...
						is_dpd) {
					DBG1(DBG_IKE, "[LGE][IWLAN] use 1 second for DPD timeout");
					timeout = (u_int32_t)(1000.0);
					aqure_wakelock = TRUE;
				} else if (strlen(operator) && strcmp(operator, "VZW") == 0 && this->initiating.retransmitted == 4) {
					// Retry config for VZW RCS 5.34
					// Timeout for 5th ike init to 8sec
			...
			/* Stop retransmission when COND_STALE */
			/* TODO: check if the interface lost case covers the COND_STALE case */
			src = this->ike_sa->get_my_host(this->ike_sa);
			if (this->ike_sa->has_condition(this->ike_sa, COND_STALE))
			{
				if (this->initiating.retransmitted > 0)
				{

					DBG1(DBG_IKE, "giving up after %d retransmits, %H to %H COND_STALE", this->initiating.retransmitted - 1, src, this->ike_sa->get_other_host(this->ike_sa));
					goto RETRANSMIT_FAILED;
				}
			}
		...
		*/
		...
		/* 2016-03-02 protocol-iwlan@lge.com LGP_DATA_IWLAN [START] */
		if (aqure_wakelock) {
			u_int32_t acquire_wake_lock_ret = acquire_wake_lock(PARTIAL_WAKE_LOCK, RETRANSMIT_DPD_WAKELOCK);
			DBG2(DBG_IKE, "[LGE][IWLAN] wakelock aquired: %s, ret=%d", RETRANSMIT_DPD_WAKELOCK, acquire_wake_lock_ret);
		}
		/* 2016-03-02 protocol-iwlan@lge.com LGP_DATA_IWLAN [END] */
...
RETRANSMIT_FAILED:
#ifdef __ANDROID__
	set_cust_setting_int(STATUS_CODE, 1081);
#endif
	return DESTROY_ME;
}
	...
	{	/* check for unsupported critical payloads */
		...
		while (enumerator->enumerate(enumerator, &payload))
		{
			...
			if (type == NOTIFY)
			{
				...
				if (is_error_notify(notify))
				{
					...
					snprintf(value, PROP_VALUE_MAX, "%d", errortype);
				...
				/* 2016-03-02 protocol-iwlan@lge.com LGP_DATA_IWLAN_SUPPORT_BACKOFF_TIMER [START] */
					...
					if (notify->get_notify_type(notify) == BACKOFF_TIMER) {
						...
						val = *(u_int16_t*)data.ptr;
                        DBG1(DBG_ENC, "notify type : BACKOFF_TIMER   read 2byte data : %d", val);
                        val = val >> 8;
                        val = val & 0x00FF;
						unit = val >> 5;
						val = val & 31;
						DBG1(DBG_ENC, "notify type : BACKOFF_TIMER   unit: %d   value: %d", unit, val);
						if (unit == 7) {
							backofftime = 0xffffffff;
						} else if (unit == 0) {
							backofftime = (u_int32_t)val * 10 * 60;
						} else if (unit == 1) {
							backofftime = (u_int32_t)val * 60 * 60;
						} else if (unit == 2) {
							backofftime = (u_int32_t)val * 10 * 60 * 60;
						} else if (unit == 3) {
							backofftime = (u_int32_t)val * 2;
						} else if (unit == 4) {
							backofftime = (u_int32_t)val * 30;
						} else if (unit == 5) {
							backofftime = (u_int32_t)val * 60;
						}
						DBG1(DBG_ENC, "notify type BACKOFF_TIMER value %d",backofftime);
					}
				}
#endif
				/* 2016-03-02 protocol-iwlan@lge.com LGP_DATA_IWLAN_SUPPORT_BACKOFF_TIMER [END] */
		...
		if (errortype != 0) {
			...
			if (len >= sizeof(conn_name)) {
				DBG1(DBG_ENC, "Failed to copy conn_name, sizeof(conn_name)=%d, len(get_name)=%d", sizeof(conn_name), len);
				len = sizeof(conn_name) -1;
			}
			...
			if (errortype == NETWORK_FAILURE ||
				...
				if (backofftime == 0xffffffff) {
					strcpy(value2,"deactivate");
				} else {
					snprintf(value2, PROP_VALUE_MAX, "%d",backofftime);
				}
				DBG1(DBG_ENC, "property_set %s %s",key2,value2);
				property_set(key2, value2);
				notify_error(value, conn_name);
			} else if (((this->initiating.type == IKE_SA_INIT) || (this->initiating.type == CREATE_CHILD_SA)) &&
				(errortype == INVALID_KE_PAYLOAD))
			{
				...
				if (pcas_is_operator("ORG", slotid) && this->retransmit_tries == 0)
				{
					DBG1(DBG_IKE, "ORG: Do not notify error to daemon attemped:[%d] before giving up [%d]",
							this->initiating.retransmitted, this->retransmit_tries);
				}
				else
				/* 2017-01-02 sy.yun@lge.com LGP_DATA_IWLAN_ALLOW_ESP_DHGROUP_WHILE_REKEY_ORG [END] */
				if ((this->initiating.retransmitted) <= this->retransmit_tries)
				{
					DBG1(DBG_IKE, "Do not notify error to daemon attemped:[%d] before giving up [%d]",
					this->initiating.retransmitted, this->retransmit_tries);
				}
				else
				{
			...
			else if (msg->get_exchange_type(msg) == INFORMATIONAL && errortype == INVALID_MESSAGE_ID ) {
				...
				if ( this->initiating.type == INFORMATIONAL ) {
					...
					while( dpd_task_enumerator -> enumerate(dpd_task_enumerator, (void*)&dpd_task)) {
						if ( dpd_task-> get_type(dpd_task) == TASK_IKE_DPD) {
							is_invalid = true;
							DBG1(DBG_IKE, "Do not notify error to daemon,dpd-invalid_message_id");
						}
					}
					dpd_task_enumerator->destroy(dpd_task_enumerator);
				}
				else if ( this->initiating.type == EXCHANGE_TYPE_UNDEFINED ) {
					DBG1(DBG_IKE, "This-initiaing type is UNDEFINED, so skip notify error-invlaid-mid");
					is_invalid = true;
				}

				if ( is_invalid ) {
					DBG1(DBG_IKE, "skip INVALID_MESSAGE_ID error");
				}
				else {
					notify_error(value, conn_name);
				}
...
/* 2016-07-02 protocol-iwlan@lge.com LGP_DATA_IWLAN_DPD_NOW [START] */
METHOD(task_manager_t, queue_dpd_now_per_conn, void,
	private_task_manager_t *this, int retransmit_tries, int retransmit_timeout)
{
	ike_mobike_t *mobike;
	...
	{
		ike_dpd_t *t_dpd = ike_dpd_create(TRUE);
		t_dpd->set_retrans(t_dpd, retransmit_tries, retransmit_timeout);
		queue_task(this, (task_t*)t_dpd);
	}
}
/* 2016-07-02 protocol-iwlan@lge.com LGP_DATA_IWLAN_DPD_NOW [END] */
...
task_manager_v2_t *task_manager_v2_create(ike_sa_t *ike_sa)
{
	...
	if (get_cust_setting_by_slotid(slotid, IKE_RETRAN_TRIES, sysvalue))
	{
		...
		DBG1(DBG_IKE, "Number of times to retransmit a packet before giving up: %d", this->retransmit_tries);
	}

	if (get_cust_setting_by_slotid(slotid, IKE_RETRAN_TO, sysvalue))
	{
		this->retransmit_timeout = atof(sysvalue);
		DBG1(DBG_IKE, "Timeout in seconds before sending first retransmit: %f", this->retransmit_timeout);
	}

	if (get_cust_setting_by_slotid(slotid, IKE_RETRAN_BASE, sysvalue))
	{
