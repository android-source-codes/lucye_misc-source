								   ...
								   bool no_dh, bool ike_auth)
{
	...
	if (this->proposal == NULL)
	{
		...
		if (pcas_is_operator("ORG", slotid))
		{
			DBG1(DBG_IKE, "no acceptable proposal found, finding again without dh");
			this->proposal = this->config->select_proposal(this->config,
					this->proposals, TRUE, private);
		}
	}
	...
	slotid = get_slotid(this->child_sa->get_name(this->child_sa));
	if (this->initiator)
	{
		...
		if (my_ts != NULL && my_ts->get_count(my_ts) > 0) {
			...
			for (i = 0; i < num ; i++)
			{
				my_ts->remove_first(my_ts, (void**)&mts);
				DBG1(DBG_IKE, "my ts : %R", mts);
				//[LGSI_DATA]:start: Its necessary to force AUTH3/AUTH4 Reponse : traffic selector intiator information of
				//TS_IPV6_ADDR_RANGE to  64bits incase of Private IID Usage in UE .
				//This issue was found when device was assigned with both v4+v6 address in DTAG DE network
				if (get_cust_setting_bool_by_slotid(slotid, FORCE_TSI_64))
				{
					enumerator_t *enumerator1 = this->ike_sa->get_virtual_ips(this->ike_sa);
					host_t *vip1;
					while (enumerator1->enumerate(enumerator1, &vip1)) {
						DBG1(DBG_IKE, "INSIDE enumerator1");
						if ((mts->get_type(mts) == TS_IPV6_ADDR_RANGE && vip1->get_family(vip1) == AF_INET6)) {
							DBG1(DBG_IKE, "FORCE set_ts_ipv6_range_64 IPV6");
							mts->set_ts_ipv6_range_64(mts);
							DBG1(DBG_IKE, "set_ts_ipv6_range_64 %R", mts);
						}
					}
					enumerator1->destroy(enumerator1);
					DBG1(DBG_IKE, "[LGSI_DATA]my ts After set_ts_ipv6_range_64 : %R",mts);
				}
				//[LGSI_DATA]:End
				if (mts->has_full_range(mts, TS_IPV4_ADDR_RANGE)) {
					DBG1(DBG_IKE, "my ts has full range ts : %R", mts);
					enumerator_t *enumerator = this->ike_sa->get_virtual_ips(this->ike_sa);
					host_t *vip;
					while (enumerator->enumerate(enumerator, &vip)) {
						if ((mts->get_type(mts) == TS_IPV4_ADDR_RANGE && vip->get_family(vip) == AF_INET) ||
								(mts->get_type(mts) == TS_IPV6_ADDR_RANGE && vip->get_family(vip) == AF_INET6)) {
							mts->set_ts_single_addr(mts, vip);
							DBG1(DBG_IKE, "my_ts is updated : %R", mts);
						}
					}
					enumerator->destroy(enumerator);
				}
				my_ts->insert_last(my_ts, (void**)mts);
			}
		}
		//original :  if (get_cust_setting_bool(FORCE_TSI_FULL)) {
		if (0)
		{
			enumerator_t *enumerator =
			...
			while (enumerator->enumerate(enumerator, &vip)) {
				...
				if (my_ts->remove_first(my_ts, (void**)&src_ts) == SUCCESS) {
					src_ts->set_ts_single_addr(src_ts, vip);
					my_ts->insert_first(my_ts, (void**)src_ts);
				}
			}
			enumerator->destroy(enumerator);
		}
		/* 2016-08-30 protocol-iwlan@lge.com LGP_DATA_IWLAN_FORCE_ADD_SUBNET_WHEN_TS_HAS_FULL_RANGE [END] */
	}
	else
	{
		...
		if (get_cust_setting_bool_by_slotid(slotid, FORCE_TSI_FULL)) {
			enumerator_t *enumerator = this->ike_sa->get_virtual_ips(this->ike_sa);
			host_t *vip;
			while (enumerator->enumerate(enumerator, &vip)) {
				traffic_selector_t *src_ts = NULL;
				if (other_ts->remove_first(other_ts, (void**)&src_ts) == SUCCESS) {
					src_ts->set_ts_single_addr(src_ts, vip);
					other_ts->insert_first(other_ts, (void**)src_ts);
				}
			}
			enumerator->destroy(enumerator);
		}
		...
		if (get_cust_setting_bool_by_slotid(slotid, FORCE_TSR_FULL)) {
			...
			while (enumerator->enumerate(enumerator, &vip)) {
				...
				//my_ts : if(fulladdr) make vip.
				traffic_selector_t *dst_ts = NULL;
				num = my_ts->get_count(my_ts);
				for(i = 0 ; i < num ; i++) {
				...
				//other_ts : if(vip) make fulladdr
				traffic_selector_t *src_ts = NULL;
				num = other_ts->get_count(other_ts);
				for(i = 0 ; i < num ; i++) {
		...
		/* 2017-01-02 sy.yun@lge.com LGP_DATA_IWLAN_FORCE_ANYADDR_TSR_WHILE_REKEY [END] */
	}
	...
	private_child_create_t *this, message_t *message)
{
	...
	{	/* no virtual IPs configured */
		...
		/* 2018-04-03 sy.yun@lge.com LGP_DATA_IWLAN REKEY_FULL [START] */
		int slotid = get_slotid(this->ike_sa->get_name(this->ike_sa));
		if (get_cust_setting_bool_by_slotid(slotid, REKEY_FULL))
		{
			if (this->reqid)
			{
				u_int8_t has_v4 = 0;
				u_int8_t has_v6 = 0;
				enumerator = this->ike_sa->get_virtual_ips(this->ike_sa);
				while (enumerator->enumerate(enumerator, &vip))
				{
				...
				if (!has_v4)
				{
					...
					list->insert_last(list, vip);
					DBG0(DBG_IKE, "FORCE FILLUP add V4_ANY");
				}
				if (!has_v6)
				{
					vip = host_create_any(AF_INET6);
					list->insert_last(list, vip);
					DBG0(DBG_IKE, "FORCE FILLUP add V6_ANY");
				}
			}
		}
		/* 2018-04-03 sy.yun@lge.com LGP_DATA_IWLAN REKEY_FULL [END] */
