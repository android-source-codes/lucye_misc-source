	...
	private_ike_delete_t *this, message_t *message)
{
	...
	if (get_cust_setting_bool(LGP_DATA_DEBUG_ENABLE_PRIVACY_LOG)) {
		...
		DBG1(DBG_IKE, "deleting IKE_SA %s[%d]",
			 this->ike_sa->get_name(this->ike_sa),
			 this->ike_sa->get_unique_id(this->ike_sa));
	}
#else
	...
	switch (this->ike_sa->get_state(this->ike_sa))
	{
		case IKE_CONNECTING:
			...
			if (this->ike_sa->get_child_count(this->ike_sa) == 0)
			{
				...
				DBG1(DBG_IKE, "notify_error for IKE_SA %s[%d] deleted, without child SA",
					 this->ike_sa->get_name(this->ike_sa),
					 this->ike_sa->get_unique_id(this->ike_sa));
				notify_error("1081", conn_name); // IWLAN_S2B_IKEV2_MSG_TIMEOUT

				set_cust_setting_int(STATUS_CODE, 1);
			}
#endif
			break;
