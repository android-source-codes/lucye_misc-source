...
static bool do_dpd(private_ike_sa_t *this)
{
	...
	{
		DBG1(DBG_IKE, "send_dpd_now this task is busy ");
		/* an exchange is in the air, no need to start a DPD check */
	}
	else
	{
		/* too long ago, initiate dead peer detection */
		DBG1(DBG_IKE, "send_dpd_now call queue_dpd sending DPD request");
	...
	private_ike_sa_t *this)
{
	...
	else
	{
		...
		if (has_condition(this, COND_STALE)) {
			DBG1(DBG_IKE, "sending DPD: diff: %d, delay: %d", diff, delay);
			delay = diff;
		}
#endif
		if (!delay || diff >= delay)
...
METHOD(ike_sa_t, send_dpd_now, status_t,
	private_ike_sa_t *this)
{
	bool task_queued = FALSE;
	DBG1(DBG_IKE, "send_dpd_now called sa name : %s",get_name(this));
	if (this->state == IKE_PASSIVE)
	{
		return INVALID_STATE;
	}
...
METHOD(ike_sa_t, send_dpd_now_per_conn, status_t,
		private_ike_sa_t *this, int retransmit_tries, int retransmit_timeout)
{
	bool task_queued = FALSE;
	DBG1(DBG_IKE, "send_dpd_now_per_conn called sa name: %s, tires: %d, timeout: %d", get_name(this), retransmit_tries, retransmit_timeout);
	if (this->state == IKE_PASSIVE)
	{
		return INVALID_STATE;
	}
	...
	if (this->task_manager->busy(this->task_manager))
	{
		DBG1(DBG_IKE, "send_dpd_now_per_conn this task is busy ");
		/* an exchange is in the air, no need to start a DPD check */
	}
	else
	{
		/* too long ago, initiate dead peer detection */
		DBG1(DBG_IKE, "send_dpd_now_per_conn call queue_dpd sending DPD request");
		this->task_manager->queue_dpd_now_per_conn(this->task_manager, retransmit_tries, retransmit_timeout);
		task_queued = TRUE;
	}
...
static void set_dscp(private_ike_sa_t *this, packet_t *packet)
{
	...
	if (ike_cfg)
	{
		...
		packet->set_dscp(packet, dscp_value);
		/* 2017-01-02 sy.yun@lge.com LGP_DATA_IWLAN_SET_DSCP_ORG [END] */
	}
}
	...
	private_ike_sa_t *this)
{
	...
	if (get_cust_setting_bool_by_slotid(slotid, REAUTH_DELETE_ONLY)) {
		...
		DBG1(DBG_IKE, "reauthenticating IKE_SA %s[%d] do set_condition(COND_REAUTHENTICATING)",
			 get_name(this), this->unique_id);
		set_condition(this, COND_REAUTHENTICATING, TRUE);
	}
#else
	...
	if (this->task_manager->retransmit(this->task_manager, message_id) != SUCCESS)
	{
		...
		switch (this->state)
		{
			case IKE_CONNECTING:
			{
				...
				strcpy(conn_name, (this->public.get_name(&this->public)));
				notify_error("1081", conn_name);

				if ((strcmp(status, "0") == 0) && (property_set("net.wo.statuscode", "1081") != 0))
				{
