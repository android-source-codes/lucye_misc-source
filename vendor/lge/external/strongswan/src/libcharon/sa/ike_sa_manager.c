...
METHOD(ike_sa_manager_t, get_sa_by_ikeid_rekey, ike_sa_t*,
	private_ike_sa_manager_t *this, ike_sa_id_t *ike_sa_id)
{
	...
	u_int segment;
	DBG1(DBG_MGR, "[LGSI_DATA] get_sa_by_ikeid_rekey for IKE SA");
	...
	while (enumerator->enumerate(enumerator, &entry, &segment))
	{
		if (wait_for_entry(this, entry, segment))
		{
			...
			if (ike_sa)
			{
				DBG1(DBG_MGR, "[LGSI_DATA] get_sa_by_ikeid_rekey IKE_SA %s[%u] successfully Returned",
	...
	private_ike_sa_manager_t *this, ike_sa_t *ike_sa)
{
		...
		entry->my_id == NULL && entry->other_id == NULL)
	{
		...
		auth_cfg_t *cfg;
		cfg = ike_sa->get_auth_cfg(ike_sa, TRUE);
		...
		if (host) {
			...
			entry->other_id = identification_create_from_string(peer_addr);
		} else {
...
ike_sa_manager_t *ike_sa_manager_create()
{
	...
	INIT(this,
		.public = {
/* 2016-02-26 protocol-iwlan@lge.com LGP_DATA_IWLAN [START] */
			.get_sa_by_ikeid_rekey = _get_sa_by_ikeid_rekey,
/* 2016-02-26 protocol-iwlan@lge.com LGP_DATA_IWLAN [END] */
			.checkout = _checkout,
