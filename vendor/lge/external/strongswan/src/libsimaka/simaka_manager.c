...
METHOD(simaka_manager_t, card_get_triplet, bool,
	...
	char rand[SIM_RAND_LEN], char sres[SIM_SRES_LEN], char kc[SIM_KC_LEN], char *name)
{
	...
	{
		if (card->get_triplet(card, id, rand, sres, kc, name))
		{
...
METHOD(simaka_manager_t, card_get_quintuplet, status_t,
	...
	char res[AKA_RES_MAX], int *res_len, char *name)
{
	enumerator_t *enumerator;
	simaka_card_t *card;
	...
	while (enumerator->enumerate(enumerator, &card))
	{
		status = card->get_quintuplet(card, id, rand, autn, ck, ik, res, res_len, name);
		switch (status)
