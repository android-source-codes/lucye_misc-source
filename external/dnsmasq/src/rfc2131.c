  ...
  */
  ...
  switch (mess_type)
      ...
      else 
	{
	  ...
	  // LGE_UPDATE_S, 20160125, check context for p2p interface
	  if (iface_name && strncmp(iface_name, "p2p", 3) == 0) {
	    // p2p use 192.168.49.xxx
	    if (context && ((ntohl(context->start.s_addr) & 0xff00) >> 8) != 0x31) {
	      my_syslog(MS_DHCP | LOG_WARNING, _("DHCPDISCOVER(%s) context is not for p2p [%s]"),
	        ((netid && netid->net) ? netid->net : "null"), inet_ntoa(context->start));
	      struct in_addr p2p_addr;
	      p2p_addr.s_addr = inet_addr("192.168.49.2");
	      if (!(context = narrow_context(context, p2p_addr, netid))) {
	        my_syslog(MS_DHCP | LOG_ERR, _("DHCPDISCOVER fail to get a p2p context"));
	        //message = _("context is not found for p2p");
		return 0;
	      }
	    }
	  }
	  // LGE_UPDATE_E, 20160125, check context for p2p interface
