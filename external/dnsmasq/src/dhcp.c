		     ...
		     struct dhcp_netid *netids, time_t now)   
{
  ...
  for (pass = 0; pass <= 1; pass++)
	...
	{
// [START][LGE_WIFI][COMMON][minsu.choi@lge.com] : [SPEC] Support only 255.255.255.0 during Hotspot mode
// P OS: In DBS mode, change from softap0 to wlan1 interface
      if (strcmp(netids->net, "wlan1") == 0) {
        // [2017TD#31593] If the Hotspot device turn on Wi-Fi Sharing after connected AP that supports 255.255.0.0 netmask,
        // It send 192.168.42.x IP address in DHCP-OFFER packet to the Client device.
        // So, Ignore 255.255.0.0 netmask and only support 255.255.255.0 netmask in Hotspot mode.
        if (c->netmask.s_addr != inet_addr("255.255.255.0")) {
          my_syslog(LOG_DEBUG, "Wi-Fi Sharing support only 255.255.255.0 netmask.");
          continue;
        }
      }
// [END][LGE_WIFI][COMMON][minsu.choi@lge.com] : [SPEC] Support only 255.255.255.0 during Hotspot mode
