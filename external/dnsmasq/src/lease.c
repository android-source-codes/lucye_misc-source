...
void lease_init(time_t now)
{
  ...
  if (daemon->options & OPT_LEASE_RO)
  ...
  else
    {
      ...
      //LGE_WiFi_S, protocol-wifi@lge.com, Common Mobile Hotspot
      //can not read dnsmasq.leases from framework
      if (chown(daemon->lease_file, AID_SYSTEM, AID_SYSTEM) < 0) {
          my_syslog(LOG_INFO, _("lease file change mode fail"));
      } else {
          my_syslog(LOG_INFO, _("lease file change mode success"));
      }
      //LGE_WiFi_E, protocol-wifi@lge.com, Common Mobile Hotspot
  ...
  if (name)
      ...
      for (lease_tmp = leases; lease_tmp; lease_tmp = lease_tmp->next)
    {
      if (daemon->options & OPT_DHCP_FQDN)
        ...
        continue;
        // LGE_CHANGE_S protocol-wifi@lge.com by 2lee, 20180809, modify show device list *
        /* http://mlm.lge.com/di/browse/STORM-3238 */
          if (lease->hwaddr_len != 0 && memcmp(lease_tmp->hwaddr, lease->hwaddr, lease->hwaddr_len) != 0)
              continue;
        // LGE_CHANGE_E protocol-wifi@lge.com by 2lee, 20180809, modify show device list *
        }
